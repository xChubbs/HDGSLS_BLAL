...
for i=1:5
    % Addition of element
    listT{i} = load_mesh('cubeD' + string(i) + '.msh');
end
...
for k=1:4
    ...
    for i=1:size(listT, 2)
        ...
        ErrorU1     =[]; ErrorUhat1   =[]; ErrorLambda1 =[];
    
        ErrorU2     =[]; ErrorUhat2   =[]; ErrorLambda2 =[];
        ...
        [Etav_free_1, lambda_1] = eigs(M(free, free), K(free, free), 1, (1 + 1 + 1)*pi^2);
        
        [Etav_free_2, lambda_2] = eigs(M(free, free), K(free, free), 1, (2^2 + 2^2 + 2^2)*pi^2);
        
        % Solution of the free dof
        ...
        ErrorLambda1 = [ErrorLambda1 abs(((1^2 + 1^2 + 1^2)*pi^2) - lambda(1)) / (3*pi^2)];
        ErrorLambda2 = [ErrorLambda2 abs(((2^2 + 2^2 + 2^2)*pi^2) - lambda(2)) / (12*pi^2)];

        disp('   - El error reportado en lambda es:')
        disp(ErrorLambda1(end))
    end
    ...
    disp([ErrorU1' ErrorU2' ErrorUhat1' ErrorUhat2' ErrorLambda1' ErrorLambda2']);

    % Saving of values
    disp('   - Guardado de tablas de ratas y errores')

    % Generation of mat structure
    name_mat = 'box_rat_err_k_' + string(k) + '.mat';

    rat_err_k_struct.rateU1 = rateU1;
    rat_err_k_struct.rateU2 = rateU2;
    rat_err_k_struct.rateEta1 = rateEta1;
    rat_err_k_struct.rateEta2 = rateEta2;
    rat_err_k_struct.rateLambda1 = rateLambda1;
    rat_err_k_struct.rateLambda2 = rateLambda2;

    rat_err_k_struct.ErrorU1 = ErrorU1;
    rat_err_k_struct.ErrorU2 = ErrorU2;
    rat_err_k_struct.ErrorUhat1 = ErrorUhat1;
    rat_err_k_struct.ErrorUhat2 = ErrorUhat2;
    rat_err_k_struct.ErrorLambda1 = ErrorLambda1;
    rat_err_k_struct.ErrorLambda2 = ErrorLambda2;
